...
import { ChevronLeft, ChevronRight, SlidersHorizontal, LayoutGrid, List, Search } from "lucide-react";
...
const JobsPageContent = () => {
    ...
    const handleFilterChange = (type: string, value: string) => {
        setFilters(prev => {
            const currentValues = (prev[type as keyof JobFilters] as string)?.split(",").filter(Boolean) || [];
                ...
                <div className="flex flex-col lg:flex-row gap-12">
                    ...
                    <aside className="hidden lg:block w-72 flex-shrink-0">
                        <JobFilterSidebar
                            ...
                            selectedCategories={filters.categoryId?.split(",").filter(Boolean) || []}
                            selectedJobTypes={filters.jobType?.split(",").filter(Boolean) || []}
                            selectedEmploymentTypes={filters.employmentType?.split(",").filter(Boolean) || []}
                    ...
                    <div className="lg:hidden flex items-center justify-between mb-6">
                        <Sheet>
                            <SheetTrigger asChild>
                                <Button variant="outline" className="flex items-center gap-2 font-display">
                        ...
                        <p className="text-[#515B6F] font-epilogue text-sm">
                            Showing <span className="text-[#2D2D2D] font-bold">{startResult}-{endResult}</span> of <span className="text-[#2D2D2D] font-bold">{totalResults}</span> results
                        </p>
                    </div>
                    ...
                    <main className="grow space-y-8">
                        <div className="flex items-center justify-between">
                            <div className="space-y-1">
                                <h2 className="text-3xl font-bold font-clash text-[#2D2D2D]">All Jobs</h2>
                        ...
                        {isLoading ? (
                            ...
                            <div className="text-center py-20 border-2 border-dashed border-zinc-100 rounded-none bg-zinc-50/30">
                                <div className="max-w-xs mx-auto space-y-4">
                                    ...
                                    <h3 className="text-xl font-bold font-clash text-[#2D2D2D]">No jobs found</h3>
                                    <p className="text-[#515B6F] font-epilogue">
                                        We couldn't find any jobs matching your current filters. Try adjusting them or search for something else.
                                    </p>
                                    <Button
                                        variant="link"
