...
const JobListItem = ({ job }: JobListItemProps) => {
  ...
  return (
    <div className="bg-white p-6 border border-zinc-100 hover:border-[#4640DE] hover:shadow-xl transition-all group flex flex-col lg:flex-row lg:items-center gap-6">
      <div className="w-16 h-16 relative shrink-0 bg-white rounded-lg p-2 border border-zinc-50 group-hover:border-[#4640DE]/20">
        <Image
          src={logoSrc}
      ...
      <div className="flex flex-col gap-4 min-w-37.5">
        <Link href={`/jobs/${job.id}`}>
          <Button className="w-full bg-[#4640DE] hover:bg-[#3b36c0] text-white font-bold font-epilogue py-6 shadow-lg shadow-[#4640DE]/20 cursor-pointer">
            View Details
          </Button>
        </Link>
